...
export class EntregasService {
  ...
  async createWithFiles(createEntregaDto: CreateEntregaDto, files: Express.Multer.File[]): Promise<Entrega> {
    ...
    const archivos = (files || []).map(f => {
      ...
      return {
        ...
        mimetype: f.mimetype,
        size: f.size,
        path: f.path || ((f as any).destination ? path.join((f as any).destination, filename) : null),
        url,
        uploadedAt: new Date()
      };
    });
  ...
  async create(createEntregaDto: CreateEntregaDto, files?: Express.Multer.File[]): Promise<Entrega> {
  ...
  async findByActividad(actividadId: string): Promise<Entrega[]> {
    ...
    this.logger.debug(`findByActividad -> found ${docs.length} documentos (searchId=${qId})`);
    return docs;
  }
