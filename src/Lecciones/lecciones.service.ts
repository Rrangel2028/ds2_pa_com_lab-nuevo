...
export class LeccionesService {
  ...
  async create(createLeccioneDto: CreateLeccionesDto): Promise<Lecciones> {
    ...
    try {
      const unidades = createLeccioneDto.Unidades || createLeccioneDto.Unidades || [];
      if (Array.isArray(unidades) && unidades.length > 0) {
        await Promise.all(
          unidades.map((unitId: string) =>
            this.unidadesModel.findByIdAndUpdate(
              unitId,
      ...
      console.error('No se pudo actualizar Unidades con la nueva lección:', err);
    }
